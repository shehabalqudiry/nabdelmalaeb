...
@section('title', __('المشرفين') . ' | ' . __('تعديل مشرف'))
@section('content')
<div class="row">
    <!-- left column -->
    <div class="col-md-8 m-auto">
        ...
        <div class="card mt-5">
            <div class="card-header">
                <h3 class="card-title">{{ __('تعديل مشرف') }}</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="role">
                            @foreach ($roles as $role)
                                <option {{ $user->hasRole($role->name) ? 'selected' : '' }} value="{{ $role->name }}">{{ $role->display_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
                                        ...
                                        @foreach ($models as $key_model => $m)
                                            <div class="tab-pane {{ $key_model == 'users' ? 'active' : '' }}" id="tab_{{ $key_model }}">
                                                @foreach ($perm as $key_perm => $p)
                                                    <label class="m-3"><input type="checkbox" name="permissions[]" {{ $user->hasPermission($key_model . '_' . $key_perm) ? 'checked' : '' }}
