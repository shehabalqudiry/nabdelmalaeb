...
@section('title', __('الإعلانات') . ' | ' . __('تعديل إعلان'))
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-8 m-auto">
            ...
            <div class="card mt-5">
                <div class="card-header">
                    <h3 class="card-title">{{ __('تعديل إعلان') }}</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" action="{{ route('dashboard.banners.update', $banner->id) }}" method="POST"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="{{ $banner->name }}" class="form-control" id="name"
                                placeholder="{{ __('اسم الإعلان') }}">
                        </div>
                        <div class="form-group">
                            <label for="text">{{ __('وصف الإعلان') }}</label>
                            <input type="text" name="text" value="{{ $banner->text }}" class="form-control" id="name"
                                placeholder="{{ __('وصف الإعلان يتم وضعه بدلا من الصورة اذا كانت غير موجوده') }}">
                        </div>
                        <div class="form-group">
                            <label for="url">{{ __('الرابط') }}</label>
                            <input type="text" name="url" value="{{ $banner->url }}" class="form-control" id="name"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="status" name="status">
                                ...
                                <option @if($banner->status == 0) selected @endif value="0">{{ __('غير نشط') }}</option>
                                <option @if($banner->status == 1) selected @endif value="1">{{ __('نشط') }}</option>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-12 col-md-9">
                                <select class="form-control" id="place" name="place">
                                    <option>{{ __('أمكان الإعلان ظهور الإعلان في الاماكن المخصصة') }}</option>
                                    @foreach($places as $i => $p)
                                    <option @if($banner->place == $i) selected @endif value="{{ $i }}">{{ __($p) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        <div class="form-group mt-4">
            ...
            <div class="col col-md-4 text-center">
                <div class="card banner_image_preview">
                    <img src="{{ $banner->image_path }}" class="banner_image_preview card-img-top img-preview" width="300">
                  </div>
            </div>
        </div>
    </div>
